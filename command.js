...
const { checkForUpdates } = require("./bin/services/checkForUpdates");

async function checkForUpdatesMiddleware() {
  const deployerPackageJsonPath = path.resolve(__dirname, "package.json");
  await checkForUpdates(deployerPackageJsonPath);
}
